...
export function CodePanel({
  ...
  return (
    <div className="flex flex-col h-full rounded-xl border border-border overflow-hidden bg-white shadow-sm">
      ...
      <div className="flex-1 overflow-auto scrollbar-thin">
        <div className="flex min-h-full">
          <div className="shrink-0 py-4 pl-4 pr-3 text-right select-none border-r border-border/60 bg-secondary/20">
            {lines.map((_, i) => (
              <div
                key={i}
                className="text-[12px] leading-[22px] text-muted-foreground/50 font-mono"
              >
                {i + 1}
              </div>
            ))}
          ...
          <div className="flex-1 relative">
            {readOnly ? (
              <pre className="p-4 font-mono text-[13px] leading-[22px] text-foreground/90 whitespace-pre overflow-x-auto">
                {code}
              </pre>
            ) : (
              <textarea
                ...
                className="w-full h-full p-4 font-mono text-[13px] leading-[22px] text-foreground/90 bg-transparent resize-none outline-none whitespace-pre"
                spellCheck={false}
              />
            )}
          </div>
